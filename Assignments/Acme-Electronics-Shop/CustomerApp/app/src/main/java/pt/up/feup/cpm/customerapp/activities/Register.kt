...
class Register : AppCompatActivity() {
    ...
    val card_validity_month by lazy { findViewById<EditText>(R.id.card_validity_month) }
    val card_validity_year by lazy { findViewById<EditText>(R.id.card_validity_year) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        register_button.setOnClickListener {
            ...
            try {
                val type = when (card_type.checkedRadioButtonId) {
                    R.id.credit_card -> "Credit"
                    R.id.debit_card -> "Debit"
                ...
                val month = parseInt(card_validity_month.text.toString())
                val year = parseInt(card_validity_year.text.toString())
                if (month > 12 || month < 1)
                    throw Exception("Invalid card validity")

                val cardValidity = "$month/$year"
                js.accumulate("cardValidity", cardValidity)
                ...
                Toast.makeText(this@Register, e.toString(), Toast.LENGTH_LONG).show()
            }
        }
    }
